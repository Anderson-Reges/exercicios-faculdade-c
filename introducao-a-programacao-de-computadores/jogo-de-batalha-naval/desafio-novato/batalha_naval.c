...
int main(void) {
  ...
  if (ship1_start_row < 0 || ship1_start_row >= 10 || ship1_start_col < 0 || ship1_start_col + 3 > 10) {
    printf("Erro: As coordenadas ou o tamanho do navio 1 são inválidos.\n");
    return 1;
  }

  // Posicionar o navio 1 no tabuleiro
  for (int i = 0; i < 3; i++) {
  ...
  // Verificar sobreposição com o navio 1
  for (int i = 0; i < 3; i++) {
    if (board[ship2_start_row + i][ship2_start_col] != NULL) {
      printf("Erro: O navio 2 se sobrepõe ao navio 1.\n");
      return 1;
    }
  }

  // Posicionar o navio 2 no tabuleiro
  for (int i = 0; i < 3; i++) {
